...
def triton_int8_linear(x: torch.Tensor, weight: torch.Tensor, weight_scale, bias=None, compute_dtype=None, use_fp32_output: bool = False):
    ...
    if not isinstance(weight_scale, torch.Tensor):
        weight_scale = torch.tensor(weight_scale, device=x.device, dtype=torch.float32).reshape(-1)
    else:
        weight_scale = weight_scale.flatten().to(dtype=torch.float32, device=x.device)

    has_bias = bias is not None
    bias_ptr = bias if has_bias else x
    has_per_channel_scale = weight_scale.numel() > 1
    kernel_name = pick_gemm_kernel(x.device)
    ...
    if kernel_name == "ampere":
        ...
        _int8_gemm_dequant_ampere[grid](
            a_ptr=x_int8,
            b_ptr=weight_packed,
            c_ptr=output,
            a_scale_ptr=x_scale,
...
def triton_hadamard_quip_linear(
    ...
    actual_K = x_for_quant.shape[1]
    weight_K = weight.shape[1]
    if actual_K != weight_K:
        if _HADAMARD_DIAGNOSTICS:
            print(f"[DIAG SHAPE MISMATCH] x_for_quant K={actual_K}, weight K={weight_K}")
        # Pad x_for_quant to match weight K if needed
        if actual_K < weight_K:
            x_for_quant = torch.nn.functional.pad(x_for_quant, (0, weight_K - actual_K))
        # Or use the smaller K
        elif actual_K > weight_K:
            x_for_quant = x_for_quant[:, :weight_K]
            
    # Delegate the Quantization + INT8 GEMM math directly to triton_int8_linear.
    # Note: we pass bias=None here because bias in QuIP is traditionally 
    # applied *after* the inverse Hadamard transform is applied.
    output = triton_int8_linear(
        x=x_for_quant,
        weight=weight,
